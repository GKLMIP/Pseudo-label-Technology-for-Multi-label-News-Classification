...
class TweetBert(pl.LightningModule):
    ...
    def _compute_loss(self, start_logits, batch, indexs):
        loss_fn = nn.MultiLabelSoftMarginLoss(weight=torch.tensor(label_weight).to('cuda'))
        start_loss = loss_fn(start_logits[indexs,:], batch['sentiment'][indexs,:])
    ...
    def training_step(self, batch, batch_nb):
        ...
        real = []
        pseduo = []
        for nu,tmp in enumerate(type):
            if tmp == 1:
                real.append(nu)
            else:
                pseduo.append(nu)

        train_loss1 = self._compute_loss(start_logits, batch, real)
        # print(train_loss1)
        nan1 = torch.isnan(train_loss1).item()
        train_loss2 = self._compute_loss(start_logits, batch, pseduo) / 10
        # print(train_loss2)
        nan2 = torch.isnan(train_loss2).item()
        if nan1 == True:
    ...
    def validation_step(self, batch, batch_nb):
        ...
        valid_loss = self._compute_loss(start_logits, batch, [i for i in range(len(batch))])
        m = nn.Softmax()
        pred = []
...
# pseduo 1.0 weight
# label_num = [882, 115, 1521, 3512, 115, 51, 148, 1410]
# pseduo 2.0 weight
# label_num = [935, 137, 1526, 3670, 124, 59, 166, 1484]

# original weight
label_weight = [log(6702/ i) for i in label_num]
# pseduo 1.0 weight
# label_weight = [log(7754/ i) for i in label_num]
# # pseduo 2.0 weight
# label_weight = [log(8101/ i) for i in label_num]
print(label_weight)
...
with open('pseduo_with_label_2.0.json') as f:
  train_data = json.load(f)
...
train_df.columns = ['text_a','label','pseduo']
...
valid_df.columns = ['text_a','label','pseduo']
start = time.time()
...
# model.model = torch.load('run9/fold0_epoch15.pt')
trainer = pl.Trainer(
